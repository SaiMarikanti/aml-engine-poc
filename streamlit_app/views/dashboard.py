...
def render_dashboard():
    ...
    with c_bot2:
        st.markdown('<div style="font-weight: 800; font-size: 0.95rem; color: #1E3A8A; margin-bottom: 12px; letter-spacing: 0.02em;">TOP RISKY ACCOUNTS</div>', unsafe_allow_html=True)
        try:
    ...
    hdr_html = textwrap.dedent("""
        <div class="neo-card-sm" style="margin-top: 20px; margin-bottom: 14px; padding: 16px 22px;">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <div>
                    <span style="font-weight: 800; font-size: 1.05rem; color: #1E3A8A; letter-spacing: 0.02em;">RECENT HIGH-RISK ALERTS</span>
                    <span style="display: inline-block; margin-left: 8px;" class="badge-indigo">REAL-TIME SURVEILLANCE</span>
                </div>
                <span style="font-size: 0.8rem; color: #64748B;">Generated by Databricks Rule, GraphFrames, and ML Pipelines</span>
            </div>
        </div>
    """).strip()
    ...
    try:
        ...
        if not recent_alerts.empty and "ALERT_ID" in recent_alerts.columns:
            recent_alerts = recent_alerts.drop_duplicates(subset=["ALERT_ID"]).head(6)
    except Exception:
        recent_alerts = pd.DataFrame()
    
    def _render_pattern_badge(pattern: str) -> str:
        p = str(pattern).lower()
        if "cycle" in p:
            return '<span class="badge-violet">CYCLE</span>'
        elif "fan_in" in p or "fan-in" in p:
            return '<span class="badge-teal">FAN-IN</span>'
        elif "fan_out" in p or "fan-out" in p:
            return '<span class="badge-indigo">FAN-OUT</span>'
        elif "high" in p or "value" in p:
            return '<span class="badge-amber">HIGH VALUE</span>'
        elif "bipartite" in p:
            return '<span class="badge-emerald">BIPARTITE</span>'
        return f'<span class="badge-indigo">{str(pattern).upper()}</span>'

    def _render_detection_chip(det: str) -> str:
        d = str(det).lower()
        if "rule" in d:
            return '<span style="font-size: 0.76rem; font-weight: 600; color: #1E3A8A; background: #EEF2F6; padding: 3px 8px; border-radius: 6px; border: 1px solid #CFD8DC;">⚡ Rule Engine</span>'
        elif "graph" in d:
            return '<span style="font-size: 0.76rem; font-weight: 600; color: #6D28D9; background: #F5F3FF; padding: 3px 8px; border-radius: 6px; border: 1px solid #DDD6FE;">🕸 Graph Motif</span>'
        elif "ml" in d or "model" in d:
            return '<span style="font-size: 0.76rem; font-weight: 600; color: #2563EB; background: #EFF6FF; padding: 3px 8px; border-radius: 6px; border: 1px solid #BFDBFE;">🤖 ML Model</span>'
        return f'<span style="font-size: 0.76rem; font-weight: 600; color: #475569; background: #F1F5F9; padding: 3px 8px; border-radius: 6px; border: 1px solid #E2E8F0;">🔍 {det}</span>'

    if not recent_alerts.empty:
        tbl_hdr_html = textwrap.dedent("""
            <div style="display: grid; grid-template-columns: 1.0fr 1.3fr 1.4fr 1.2fr 1.0fr 1.2fr 1.3fr; 
                        font-size: 0.74rem; font-weight: 800; color: #64748B; text-transform: uppercase; 
                        letter-spacing: 0.05em; border-bottom: 2px solid #CBD5E1; padding-bottom: 8px; margin-bottom: 10px;">
                <div>Alert ID</div>
        ...
        """).strip()
        st.markdown(tbl_hdr_html, unsafe_allow_html=True)
        
        for idx, alert in recent_alerts.iterrows():
            a_id = alert['ALERT_ID']
            c_al1, c_al2, c_al3, c_al4, c_al5, c_al6, c_al7 = st.columns([1.0, 1.3, 1.4, 1.2, 1.0, 1.2, 1.3])
            
            with c_al1:
                st.markdown(f'<span class="code-pill">AL{a_id}</span>', unsafe_allow_html=True)
            with c_al2:
                st.markdown(_render_pattern_badge(alert["ALERT_TYPE"]), unsafe_allow_html=True)
            with c_al3:
                st.markdown(_render_detection_chip(alert["DETECTION_ENGINE"]), unsafe_allow_html=True)
            with c_al4:
                st.markdown(f'<span style="font-weight: 700; font-size: 0.88rem; font-family: \'JetBrains Mono\', monospace; color: #0F172A;">₹{alert["TX_AMOUNT"]:,.2f}</span>', unsafe_allow_html=True)
            with c_al5:
            ...
            with c_al7:
                if st.button("Inspect →", key=f"dash_inv_{a_id}_{idx}", use_container_width=True):
